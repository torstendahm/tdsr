...
figname = current_dir / "../plots_tdsr/fig4ab"
...
sstep = [-2.0, -4.0]  # stress steps applied at time tstep, in MPa ( formerly Sstepvalues = (-2.0, -4.0) )
tstep = 0.*hours
tstep = 0.01*hours

tstart = -1.*hours   # T0 = -1.0     # simulation start
tend   = 10.*hours   # T1 = 10.0     # simulation end
...
for i, Sstep in enumerate(sstep):
    ...
    loading = StepLoading(_config=tdsr.config, strend=strend, sstep=Sstep, tstep=tstep, taxis_log=0, deltat=deltat, tstart=tstart, tend=tend)
    config, t, chiz, cf, r, xn = tdsr(loading=loading, chi0=chi0, t0=t0, depthS=depthS, deltaS=deltaS, sigma_max=sigma_max, iX0switch=0, deltat=deltat, taxis_log=0, tstart=tstart, tend=tend)
    cfs[i,:]    = cf[:]
    r_tdsr[i,:] = r[:]
    ...
    loading = StepLoading(_config=rsd.config, strend=strend, sstep=Sstep, tstep=tstep, deltat=deltat, tstart=tstart, tend=tend)
    config, t, cf_shadow, cf, r, xn = rsd(loading=loading, chi0=chi0, depthS=depthS, deltat=deltat, tstart=tstart, tend=tend)
    r_rsd[i,:] = r

    loading = StepLoading(_config=cfm.config, strend=strend, sstep=Sstep, tstep=tstep, deltat=deltat, tstart=tstart, tend=tend)
    config, t, chiz, cf, r, xn = cfm(loading=loading, chi0=chi0, deltat=deltat, tstart=tstart, tend=tend)
    r_lcm[i,:] = r
...
for i, Sstep in enumerate(sstep):
    ...
    ax[0].plot(t, cfs[i,:], c='k', ls='solid', lw=2, alpha=al[i], label=r'$\Delta\sigma_c / \delta\sigma$=%.1f' % (Sstep))
...
ax[0].legend()
ax[0].text(-0.18,.93, '(a)', fontsize=14, color='k', horizontalalignment='left', transform=ax[0].transAxes)

for i, Sstep in enumerate(sstep):
    if i==0:
        ax[1].plot(t, r_tdsr[i,:], c='b', ls='solid', lw=3, alpha=al[i], label='TDSR, RS')
...
ax[1].legend()
ax[1].text(-0.18,.93, '(b)', fontsize=14, color='k', horizontalalignment='left', transform=ax[1].transAxes)
