...
r_cfm  = np.zeros((ns,ntlog))
#r_rsm  = np.zeros((ns,ntlog))
r_rsd  = np.zeros((ns,ntlog))
...
for k in range(ns):
    ...
    #loading = StepLoading(_config=cfm.config, strend=strend, sstep=sstep[k], tstep=tstep, taxis_log=taxis_log, ntlog=ntlog, deltat=deltat, tstart=tstartl, tend=tend)
    #config, t, chiz, cf, r, xn = cfm(loading=loading, chi0=chi0, depthS=depthS, deltat=deltat, axis_log=taxis_log, ntlog=ntlog, tstart=tstartl, tend=tend)
    #r_cfm[k,:] = r

    #loading = StepLoading(_config=rsd.config, strend=strend, sstep=sstep[k], tstep=tstep, taxis_log=taxis_log, ntlog=ntlog, deltat=deltat, tstart=tstartl, tend=tend)
    #config, t, cf_shadow, cf, r, xn = rsd(loading=loading, chi0=chi0, depthS=depthS, deltat=deltat, axis_log=taxis_log, ntlog=ntlog, tstart=tstartl, tend=tend)
    #r_rsd[k,:] = r
