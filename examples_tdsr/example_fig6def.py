...
data_file = current_dir / "../data/CFSloading_KTB.dat"
figname = current_dir / "../plots_tdsr/figname = 'fig6def.png"
...
deltaS    = 0.3/500.  # increment do discretize Coulomb stress axis
...
def read_flowdata():
    data = np.loadtxt('../data/ktb_flowrates.dat', skiprows=1, usecols=(0,1))
    tq = data[:, 0]
...
def read_pressuredata():
    data = np.loadtxt('../data/ktb_HBpressure.dat', skiprows=2, delimiter=',')
    tp = data[:, 0]
    p = data[:, 1]  # [MPa]
    return tp, p
...
def calculateP(Dm2s, tgrid, rdist, tq, dq):
    ...
    ketafac = np.square(alpha) * (lambda_u + 2.0 * mu)   # Segall & Lu 2015, Eq.(4): k/eta = D * ketafac
...
# Earthquake data:
data = np.loadtxt('../data/ktb_EQ.dat')
teqall = data[:, 0]
meqall = data[:, 1]
ind = (meqall>=Mcut)
teq = teqall[ind]
...
for i in range(3):
    if i == 0: depthS=-0.3; Sshadow=3.0
    if i == 1: depthS=-0.4; Sshadow=3.5
    if i == 2: depthS=-0.5; Sshadow=4.5
...
ax[2].legend()
ax[0].text(-0.14,.93, '(d)', fontsize=14, color='k', horizontalalignment='left', transform=ax[0].transAxes)
ax[1].text(-0.14,.95, '(e)', fontsize=14, color='k', horizontalalignment='left', transform=ax[1].transAxes)
ax[2].text(-0.14,.95, '(f)', fontsize=14, color='k', horizontalalignment='left', transform=ax[2].transAxes)
