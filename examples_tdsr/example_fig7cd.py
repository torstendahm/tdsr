...
data_cfs  = current_dir / "../data/stresschange_morsleben.dat"
fn_obsrate = current_dir / "../data/morsleben_EQ.dat"
#fn_obsrate = current_dir / '../data/observed_rate_southregion.txt'
figname = current_dir / "../plots_tdsr/fig7cd"
...
config, t, cf_shad, cf, r, xn = trad(loading=loading, chi0=chi0, deltat=deltat, tstart=tstart, tend=tend)
...
for i in range(3):
    if i == 0: dsig=0.3; t0=0.01; dS0=3.0
    if i == 1: dsig=0.4; t0=0.01; dS0=3.5
    if i == 2: dsig=0.5; t0=0.01; dS0=4.5
    ax[1].plot(t[:], fac*r_tdsr[i,:], c=cb[i], lw=1, label=r'$\delta\sigma=%.1f$ $\zeta_\mathrm{min}=%.1f$' % (dsig, dS0))
...
ax[1].legend()
ax[0].text(-0.14,.93, '(c)', fontsize=14, color='k', horizontalalignment='left', transform=ax[0].transAxes)
ax[1].text(-0.14,.95, '(d)', fontsize=14, color='k', horizontalalignment='left', transform=ax[1].transAxes)
