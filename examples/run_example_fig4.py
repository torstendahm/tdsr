...
# hier: run_Fig4.py dubliziert, um direkt mit tdsr Implementation zu vergleichen
#--------------------------
import sys
sys.path.insert(0, "../")
...
pdf_file1 = current_dir / "../plots/fig4a"
pdf_file2 = current_dir / "../plots/figB1b"
pdf_file3 = current_dir / "../plots/fig4b"
...
taxis_log=1    # logarithmic time axis discretization
ntlog=100
tstartl = 0.01*hours
...
r_rsd   = np.zeros((2,nt))
r_cfm   = np.zeros((2,nt))
n_tdsr  = np.zeros((2,nt-1))
n_rsd   = np.zeros((2,nt-1))
n_cfm   = np.zeros((2,nt-1))
...
for i in range(2):
    ...
    loading = StepLoading(_config=tdsr.config, strend=strend, sstep=sstep[i], tstep=tstep, taxis_log=0)
    ...
    loading = StepLoading(_config=rsd.config, strend=strend, sstep=sstep[i], tstep=tstep)
...
loading = CyclicLoading(_config=tdsr.config, strend=strend, ampsin=ampsin, Tsin=Tsin)
config, t, chiz, cf, r, xn = tdsr(loading=loading, chi0=chi0, t0=t0, depthS=depthS, deltaS=deltaS, sigma_max=sigma_max, iX0switch=0, deltat=deltat, taxis_log=0, tstart=0, tend=tend)
r_tdsr_cycle = r
n_tdsr_cycle = xn
...
loading = CyclicLoading(_config=rsd.config, strend=strend, ampsin=ampsin, Tsin=Tsin)
...
ax1b.plot((t-t[nstep])/Ta, r_tdsr[i,:]/scal, linewidth=2.0, ls=lstyle[i], color='black')
#ax1b.plot((t-t[nstep])/Ta, r_rsm[i,:]/scal, linewidth=1.0, ls=lstyle[1], color='blue')
ax1b.plot((t-t[nstep])/Ta, r_rsd[i,:]/scal, linewidth=1.0, ls=lstyle[1], color='blue')
...
ax1b.plot((t-t[nstep])/Ta, r_tdsr[i,:]/scal, linewidth=2.0, ls=lstyle[i], color='black', label=r'TDSR')
#ax1b.plot((t-t[nstep])/Ta, r_rsm[i,:]/scal, linewidth=1.0, ls=lstyle[1], color='blue', label=r'RSM')
ax1b.plot((t-t[nstep])/Ta, r_rsd[i,:]/scal, linewidth=1.0, ls=lstyle[1], color='blue', label=r'RSD')
...
ax1c.plot((t[0:-1]-t[nstep])/Ta, n_tdsr[i,:]-n_tdsr[i,nstep], linewidth=1.5, ls=lstyle[i], color='gray')
#ax1c.plot((t[0:-1]-t[nstep])/Ta, n_rsm[i,:]-n_rsm[i,nstep], linewidth=0.5, ls=lstyle[1], color='blue')
ax1c.plot((t[0:-1]-t[nstep])/Ta, n_rsd[i,:]-n_rsd[i,nstep], linewidth=0.5, ls=lstyle[1], color='blue')
...
ax1c.plot((t[0:-1]-t[nstep])/Ta, n_tdsr[i,:]-n_tdsr[i,nstep], linewidth=1.5, ls=lstyle[i], color='gray', label=r'TDSR')
#ax1c.plot((t[0:-1]-t[nstep])/Ta, n_rsm[i,:]-n_rsm[i,nstep], linewidth=0.5, ls=lstyle[1], color='blue', label=r'RSM')
ax1c.plot((t[0:-1]-t[nstep])/Ta, n_rsd[i,:]-n_rsd[i,nstep], linewidth=0.5, ls=lstyle[1], color='blue', label=r'RSD')
...
ax1b.plot((t-t[nstep])/Ta, r_tdsr_cycle/scal, linewidth=2.0, ls='--', color='black')
#ax1b.plot((t-t[nstep])/Ta, r_rsm_cycle/scal, linewidth=1.0, ls=lstyle[1], color='blue')
ax1b.plot((t-t[nstep])/Ta, r_rsd_cycle/scal, linewidth=1.0, ls=lstyle[1], color='blue')
