...
pdf_file1 = current_dir / "../plots/fig3c"
...
#    i = 1
for i in range(nsb):
    loading = BackgroundLoading(_config=tdsm.config, strend=strend_background[i], deltat=deltat, taxis_log=0, ntlog=ntlog, tstart=tstart, tend=tend)
    ...
    loading = BackgroundLoading(_config=tdsr.config, strend=strend_background[i], taxis_log=taxis_log, ntlog=ntlog, tstart=tstartl, tend=tend)
    config, tr, chiz_background, cf, r, xn = tdsr(loading=loading, chi0=chi0, t0=t0, depthS=depthS, deltaS=deltaS, sigma_max=sigma_max, precision=precision, iX0switch=iX0switch, deltat=deltat, taxis_log=taxis_log, ntlog=ntlog, tstart=tstartl, tend=tend)
    r_tdsr_eq8[i,:] = r
...
for i in range(ns):
    ...
    loading = StepLoading(_config=tdsr.config, strend=strend, sstep=sstep[i], tstep=tstep, taxis_log=0, deltat=deltat, tstart=tstart, tend=tend)
    config, t, chiz, cf, r, xn = tdsr(loading=loading, chi0=chi0, t0=t0, depthS=depthS, deltaS=deltaS, sigma_max=sigma_max, precision=precision, iX0switch=0, deltat=deltat, taxis_log=0, ntlog=ntlog, tstart=tstart, tend=tend)
    r_tdsr[i,:] = r[:]
    n_tdsr[i,:] = xn[:]
...
for i in range(nsb-1):
    print(' tdsm: shape(t)=',np.shape(t[nstart+1:]),' shape(r)=',np.shape(r_tdsm_eq8[i,1:-nstart]))
    print(' log axis: shape(t)=',np.shape(tr),' shape(r)=',np.shape(r_tdsr_eq8[i,:]))
    print(' taxis_log=',taxis_log)
    print(' nstart=',nstart)
    ax.plot((t[nstart+1:]-t[nstep])/deltat, r_tdsm_eq8[i,1:-nstart]/scal1[i], linewidth=thicknum, ls='-', color=colnum)
    if taxis_log == 1:
        ax.plot(tr/deltat, r_tdsr_eq8[i,:]/scal1[i], linewidth=2.0, ls='-', color='black')
    else:
...
if taxis_log == 1:
    ax.plot(tr/deltat, r_tdsr_eq8[i,:]/scal1[i], linewidth=2.0, ls='-', color='black', label=r'TdSR')
else:
...
for i in range(ns-1):
    ...
    ax1b.plot((t[nstart:]-t[nstep])/Ta , r_tdsr[i,nstart:]/rinfty, linewidth=2.0, ls='-', color='black')
...
ax1b.plot((t[nstart:]-t[nstep])/Ta , r_tdsr[i,nstart:]/rinfty, linewidth=2.0, ls='-', color='black', label=r'TDSR')
