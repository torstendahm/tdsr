...
from tdsm.loading import StepLoading, BackgroundLoading, ExternalFileLoading
...
figname = current_dir / "../plots/fig5bc"
...
########## INPUTS:
# Earthquake data:
data = np.loadtxt('../data/groningenEQ.dat')
teq = data[:, 0]
meq = data[:, 1]
NEQ = len(teq[((teq>=T1) & (teq<=T2))])
# Pressure data:
data = np.loadtxt('../data/groningenP.dat')
tgrid = data[:, 0]
dt = tgrid[1] - tgrid[0]
...
print('time when EQ start, t00=',t00,tgrid[0], dotsigc*(t00-tgrid[0]),dotsigc_sub*(t00-tgrid[0]))
data_file = "../data/groningenCFS.dat"
np.savetxt(data_file, np.transpose([S, tgrid]), header='\n'.join(["Ascii loading file is expected to have two header lines", "1st column: Coulomb stress, 2nd column: time"]))

######## MODEL SIMULATIONS: ######

#  ------ loading assuming that faults under critical stress exists --------
loading = ExternalFileLoading(_config=tdsr.config, iascii=True, infile=data_file, scal_t=1., scal_cf=1., strend=dotsigc, c_tstart=tstart, tstart=tstart, tend=tend, deltat=dt)

# CF model with critical initial stress state:
config, t, chiz, cf, R_CF, xn = cfm(loading=loading, chi0=1.0, Sshadow=0., depthS=depthS, deltat=dt, tstart=tstart, tend=tend)
fac = NEQ / np.sum(R_CF[((tgrid[0:nt]>=T1) & (tgrid[0:nt]<=T2))]*dt) # renormalize to observed EQ-number within [T1, T2]:
R_CF *= fac

# CF model with subcritical initial stress state:
config, t, chiz, cf, R_CFsub, xn = cfm(loading=loading, chi0=1.0, Sshadow=dS0, depthS=depthS, deltat=dt, tstart=tstart, tend=tend)
fac = NEQ / np.sum(R_CFsub[((tgrid[0:nt]>=T1) & (tgrid[0:nt]<=T2))]*dt)
R_CFsub *= fac

# RS model for critical initial stress state:
config, t, chiz, cf, R_RS, xn = rsd1(loading=loading, chi0=1.0, Sshadow=0., depthS=depthS, deltat=dt, tstart=tstart, tend=tend)
fac = NEQ / np.sum(R_RS[((tgrid[0:nt]>=T1) & (tgrid[0:nt]<=T2))]*dt)
R_RS *= fac

# TDSR model for critical initial stress state:
config, t, chiz, cf, R_TDSR, xn = tdsr(loading=loading, chi0=1.0, t0=t0, depthS=depthS, Sshadow=0., deltaS=deltaS, sigma_max=sigma_max, iX0switch=0, deltat=dt, taxis_log=0, tstart=tstart, tend=tend)
...
loading = ExternalFileLoading(_config=tdsr.config, iascii=True, infile=data_file, scal_t=1., scal_cf=1., strend=dotsigc_sub, c_tstart=tstart, tstart=tstart, tend=tend, deltat=dt)

# RS model for subcritical initial stress state:
config, t, chiz, cf, R_RSsub, xn = rsd1(loading=loading, chi0=1.0, Sshadow=(dS0+dotsigc_sub*(t00-tgrid[0])), depthS=depthS, deltat=dt, tstart=tstart, tend=tend)
...
config, t, chiz, cf, R_TDSRsub_uniform, xn = tdsr(loading=loading, chi0=1.0, t0=t0, depthS=depthS, Sshadow=dS0_TDSR, deltaS=deltaS, sigma_max=sigma_max, iX0switch=1, deltat=dt, taxis_log=0, tstart=tstart, tend=tend)
...
config, t, chiz, cf, R_TDSRsub_gauss, xn = tdsr(loading=loading, chi0=1.0, t0=t0, depthS=depthS, Sshadow=0., deltaS=deltaS, sigma_max=sigma_max, iX0switch=2, Zmean=dS0mean, Zstd=dS0std, deltat=dt, taxis_log=0, tstart=tstart, tend=tend)
...
ax[1].scatter(0.5*(tbin[:-1]+tbin[1:]), nobs, c='k', marker='o', s=60, lw=3, alpha=0.5, label='observed')
...
ax[1].legend()
ax[0].text(-0.12,.93, '(b)', fontsize=14, color='k', horizontalalignment='left', transform=ax[0].transAxes)
ax[1].text(-0.12,.95, '(c)', fontsize=14, color='k', horizontalalignment='left', transform=ax[1].transAxes)
