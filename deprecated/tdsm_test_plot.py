...
def plot_paper(config, t, cf, ratez, neqz):
    ...
    nt = len(t)
    Ta = config.deltat * (num.exp(1.0) - 1.0)
    ...
    ax1a.plot(
        (t - t[config.loading.n1]) / Ta,
        cf / (tectstressrate * config.deltat),
        linewidth=2.0,
        ls="-",
        color="black",
        label=r"$\Delta\sigma/\dot{\sigma}_c\Delta t=$" + "{:d}".format(num.int(fa)),
    ...
    ax1b.plot(
        (t[40 : nt - 1] - t[config.loading.n1]) / Ta,
        ratez[40 : nt - 1] / scal,
        linewidth=3.5,
        color="red",
        label=r"$\delta \dot{\sigma}/\dot{\sigma}_c=$" + "{:d}".format(num.int(fa)),
    ...
    ax1c.plot(
        (t[40 : nt - 1] - t[config.loading.n1]) / Ta,
        neqz[40 : nt - 1] - neqz[config.loading.n1 - 1],
        linewidth=2.5,
        color="red",
        label=r"$\delta \dot{\sigma}/\dot{\sigma}_c=$" + "{:d}".format(num.int(fa)),
    )
